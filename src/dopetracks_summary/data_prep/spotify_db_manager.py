    ...
    try:
        ...
        for url in tqdm.tqdm(input_urls, desc="Processing URLs", unit="url"):
            ...
            if any(x in url for x in ["/wrapped", "/concert", "/playlist", "/socialsession", "/blend"]):
                ...
                cursor.execute(
                    """
                    INSERT OR REPLACE INTO spotify_url_cache (original_urls, normalized_url, metadata)
                    VALUES (?, ?, ?)
                    """,
                    (json.dumps([original_url]), url, json.dumps({"error": "unsupported URL"}))
                )
                conn_cache.commit()
                continue
            ...
            if len(existing_cache_data) == 1:
                existing_original_urls = json.loads(existing_cache_data['original_urls'].iloc[0])
                ...
                new_urls.append((original_url, normalized_url))
        ...
        for original_url, normalized_url in new_urls:
            match = re.search(r"spotify\.com/(track|album|artist|show|episode)/([\w\d]+)", normalized_url)
            if match:
                entity_type = match.group(1)
                spotify_urls_by_type[entity_type].append((original_url, normalized_url))
            else:
        ...
        for entity_type, url_pairs in spotify_urls_by_type.items():
            ...
            for batch_index, batch in enumerate(tqdm.tqdm(uf.batch(url_pairs, batch_size), desc=f"Processing {entity_type} batches", unit="batch")):
                original_urls, normalized_urls = zip(*batch)
                ...
                try:
                    ...
                    for item, original_url, normalized_url in zip(metadata_items, original_urls, normalized_urls):
                        all_metadata.append((item, original_url, normalized_url))
        ...
        for item, original_url, normalized_url in all_metadata:
            query = """
                SELECT original_urls FROM spotify_url_cache
                WHERE normalized_url = ?
            """
            existing_data = pd.read_sql_query(query, conn_cache, params=(normalized_url,))
            if len(existing_data) == 1:
                original_urls = json.loads(existing_data['original_urls'].iloc[0])
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?)
                """,
                (json.dumps(original_urls), normalized_url, json.dumps(item))
            )
            conn_cache.commit()
